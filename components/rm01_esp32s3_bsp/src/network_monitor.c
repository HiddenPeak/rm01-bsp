...
static esp_err_t nm_start_simple_ping(int target_index) {
    ...
    ping_config.interval_ms = 100;                  // 减少间隔至100ms
    ping_config.timeout_ms = 1000;                  // 1秒超时，提高响应速度
...
void nm_enable_fast_monitoring(bool enable) {
    if (xSemaphoreTake(nm_mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        if (enable) {
            monitoring_interval_ms = 1000;  // 快速模式：1秒间隔
            ESP_LOGI(TAG, "启用快速监控模式，监控间隔=%" PRIu32 "ms", monitoring_interval_ms);
        } else {
            monitoring_interval_ms = 3000;  // 普通模式：3秒间隔
