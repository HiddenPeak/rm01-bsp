/**
 * @file system_state_controller.c
 * @brief 系统状态控制器实现
 * 
 * 实现基于多种条件的复杂状态联动系统
 */
...
} system_state_controller_t;

// 全局控制器实例
static system_state_controller_t s_controller = {0};
...
static const int STATE_TO_ANIMATION[] = {
    ...
    ANIM_HIGH_TEMP,     // SYSTEM_STATE_HIGH_TEMP_1 (目前使用链接错误动画)
    ANIM_HIGH_TEMP,     // SYSTEM_STATE_HIGH_TEMP_2 (目前使用链接错误动画)
    ANIM_LINK_ERROR,    // SYSTEM_STATE_USER_HOST_DISCONNECTED
    ANIM_COMPUTING      // SYSTEM_STATE_HIGH_COMPUTE_LOAD (目前使用示例动画)
};

// 静态函数声明
static void system_state_monitor_task(void *pvParameters);
...
void system_state_controller_start_monitoring(void) {
    ...
    if (ret == pdPASS) {
        ESP_LOGI(TAG, "系统状态监控任务已启动");
    } else {
        s_controller.monitoring_active = false;  // 创建失败时重置标志位
        ESP_LOGE(TAG, "创建系统状态监控任务失败");
    }
}

void system_state_controller_stop(void) {
    if (!s_controller.monitoring_active) {
        ESP_LOGW(TAG, "系统状态监控未运行");
        return;
    }
    
    ESP_LOGI(TAG, "停止系统状态监控");
...
esp_err_t system_state_force_set(system_state_t state) {
    ...
    ESP_LOGI(TAG, "强制设置系统状态为: %s", system_state_get_name(state));
    return set_system_state(state);
}

void system_state_print_status(void) {
    system_state_info_t info;
    esp_err_t ret = system_state_get_info(&info);
    
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "获取系统状态信息失败");
        return;
    }
    
    ESP_LOGI(TAG, "========== 系统状态报告 ==========");
    ESP_LOGI(TAG, "当前状态: %s", system_state_get_name(info.current_state));
    ESP_LOGI(TAG, "前一状态: %s", system_state_get_name(info.previous_state));
    ...
    ESP_LOGI(TAG, "高负荷计算: %s", info.high_compute_load ? "是" : "否");    ESP_LOGI(TAG, "监控状态: %s", s_controller.monitoring_active ? "运行中" : "已停止");
    ESP_LOGI(TAG, "================================");
}

void system_state_controller_update_and_report(void) {
    ESP_LOGI(TAG, "手动更新系统状态并生成报告");
    ...
    if (new_state != s_controller.current_state) {
        ...
        if (ret == ESP_OK) {
            ...
            ESP_LOGE(TAG, "状态更新失败: %s", esp_err_to_name(ret));
        }
    } else {
        ESP_LOGI(TAG, "状态无变化，保持当前状态: %s", system_state_get_name(new_state));
    }
    
    // 打印状态报告
    system_state_print_status();
}
...
static esp_err_t set_system_state(system_state_t new_state) {
    ...
    if (xSemaphoreTake(s_controller.state_mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        // 记录状态变化
        ESP_LOGI(TAG, "系统状态变化: [%s] -> [%s]", 
                 system_state_get_name(old_state),
                 system_state_get_name(new_state));
          // ========== TODO: 需要等待动画文件更新后完善 ==========
        // 切换对应的动画
        // 
        // 当前状态：
...
static bool is_high_compute_load(void) {
    ...
    if (power_data != NULL && power_data->valid) {
        ...
        if (power_data->power > 50.0f) {
            ESP_LOGD(TAG, "检测到高功耗状态: %.2fW > 50W", power_data->power);
            return true;
        }
    }
